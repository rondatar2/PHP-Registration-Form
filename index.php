<body>
    ...
    <div class="container">
        <form action="index.php" method = get>
            ...
            <input id=sn type=text name= studNum placeholder = "Enter your Student Number" required>
            <h3>Name</h3>
            ...
            <input id="mname" type=text name= middleName placeholder = "Middle Name" required><br><br>
            ...
            <input id="course" type=text name= course placeholder = "Course" required><br><br><br>
            <h3 style="display: inline;">Grade:</h3>
            <input id="mgrade" type=text name= mgrade placeholder = "Midterm Grade" required>
            <input id="fgrade" type=text name= fgrade placeholder = "Final Grade" required><br><br><br>
            <center>
                <input id="save" class="btn" type=submit value=Save name= save><input id="reset" class="btn" type=reset value=Clear name= clear>
            </center>
        </form>
        <?php 
        if (isset($_GET['save'])){
            ...
            include('connect.php');
            $search = "Select * from register where (student_number='$sn')";
            $result = mysqli_query($con, $search);
            $count = mysqli_num_rows($result);
            if ($count == 0){
                $insert = "INSERT INTO register values($id, '$lname', '$fname', '$mname', $age, '$gender', '$course', $mgrade, $fgrade, $tgrade)";
                mysqli_query($con, $insert);
                print 'Record Save';
            }
            else print "<p>$sn is already exists!!</p>";
        }   
        ?>
